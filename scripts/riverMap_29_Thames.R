...
HUCs <- c("01100003", "01100002", "01100001") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Thames" # river name for files
river_name_plot <- "Thames" # river name for plot title
...
cities_subset[1,] <- c("CT", "New London", 41.3557, -72.0995)
...
p <- ggplot() +
  ...
  annotate("text", x=-72.6, y=41.7, label="CT", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-72.6, y=42.2, label="MA", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-71.6, y=41.7, label="RI", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-72.6, y=41.15, label="Long Island Sound", color="white", hjust=0.5, vjust=0.5, size=5, angle=20) +
