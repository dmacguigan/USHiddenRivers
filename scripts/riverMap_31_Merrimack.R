...
HUCs <- c("010700") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Merrimack" # river name for files
river_name_plot <- "Merrimack" # river name for plot title
...
cities_subset <- cities_subset[1:2,]
cities_subset[1,1] <- "MA"
cities_subset[2,] <- c("MA", "Lowell", 42.6334, -71.3162)
...
p <- ggplot() +
  ...
  annotate("text", x=-72.6, y=42.65, label="MA", color="white", hjust=0.5, vjust=0.5, size=7) +
  annotate("text", x=-72.7, y=44, label="VT", color="white", hjust=0.5, vjust=0.5, size=7) +
  annotate("text", x=-71.4, y=44.3, label="NH", color="white", hjust=0.5, vjust=0.5, size=7) +
  annotate("text", x=-70.5, y=44, label="ME", color="white", hjust=0.5, vjust=0.5, size=7) +
