...
HUCs <- c("17100105", "17100104", "17100103") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
states <- c("WA") # states spanned by the river basin (for parsing waterbodies), use abbreviations
river_name <- "Chehalis" # river name for files
river_name_plot <- "Chehalis" # river name for plot title
...
p <- ggplot() +
  ...
  geom_raster(topo_crop_test, mapping = aes(x = x, y = y, alpha = GRAY_HR_SR_OB)) +
