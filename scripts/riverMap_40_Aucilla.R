...
HUCs <- c("03110103") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Aucilla" # river name for files
river_name_plot <- "Aucilla" # river name for plot title
...
ymax <- ceiling(extents@ymax) + 0.2
extents@ymax <- ymax
ymin <- floor(extents@ymin) -0.3
extents@ymin <- ymin
xmax <- ceiling(extents@xmax) -0.2
extents@xmax <-  xmax 
xmin <- floor(extents@xmin) + 0.7
extents@xmin <- xmin 
...
cities_subset[1,] <- c("", "Aucilla Sinks", 30.200151, -83.923583)
...
p <- ggplot() +
  ...
  annotate("text", x=-84, y=30.702, label="Georgia", color="white", hjust=0.5, vjust=0.5, size=3, angle=-3) +
  annotate("text", x=-84, y=30.648, label="Florida", color="white", hjust=0.5, vjust=0.5, size=3, angle=-3) +
