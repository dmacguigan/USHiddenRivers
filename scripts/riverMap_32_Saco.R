...
HUCs <- c("01060002") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Saco" # river name for files
river_name_plot <- "Saco" # river name for plot title
...
cities_subset[1,] <- c("", "Mt. Washington", 44.2706, -71.3033)
...
p <- ggplot() +
  ...
  annotate("text", x=-71.7, y=44, label="NH", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-70.5, y=44, label="ME", color="white", hjust=0.5, vjust=0.5, size=5) +
