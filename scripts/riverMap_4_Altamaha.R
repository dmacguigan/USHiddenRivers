...
HUCs <- c("030701") # top-level vector of HUCs to match (must be strings, not numeric)
...
river_name <- "Altamaha" # river name for files
river_name_plot <- "Altamaha" # river name for plot title
...
gworld <- ggplot(data = world) +
  geom_sf(fill="black", color="gray20") +
  geom_sf(data=us_states, fill="black", color="gray20") +
  geom_rect(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, 
            fill = NA, colour = "white", size = 0.8) +
  coord_sf(xlim = c(-130, -65), ylim = c(25, 55), expand = FALSE, datum=NA) +
  theme(panel.border = element_rect(colour = "white", fill=NA, size=4), panel.grid.major = element_blank(),
...
p <- ggplot() +
  ...
  annotate("label", x=-82.7, y=33.4, label="Fall Line", color="red", hjust=0, size=6) +
