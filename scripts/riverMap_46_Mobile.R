...
HUCs <- c("0315", "0316") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Mobile" # river name for files
river_name_plot <- "Mobile" # river name for plot title
...
target_watershed <- target_watershed[-which(as.logical(rowSums(sapply(c("0316020503", "0316020501", "0316020502"), startsWith, x=huc10s)))),] # remove some HUCs
...
in_ws_rivers <- over(target_rivers_ws, target_watershed)
target_rivers_ws <- target_rivers_ws[!is.na(in_ws_rivers$dummy),]
...
cities_subset[1,] <- c("Alabama", "Mobile", 30.6954, -88.0399)
cities_subset[2,] <- c("Alabama", "Birmingham", 33.5186, -86.8104)
cities_subset[3,] <- c("Alabama", "Montgomery", 32.3792, -86.3077)
...
gworld <- ggplot(data = world) +
  geom_sf(fill="gray20", color="white", size=0.25) +
  geom_sf(data=us_states, fill="gray20", color="white", size=0.25) +
  geom_rect(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, 
            fill = NA, colour = "red", size = 0.65) +
...
p <- ggplot() +
  ...
  geom_label_repel(data = cities_subset[1,], aes(y=lat, x=long, label=city),
                   box.padding   = 0.35, size = 3, alpha=0.9, segment.alpha=1, point.padding = 0.5,
                   ylim=c(30, 32), 
                   xlim=c(-90, -88.3),
                   segment.color = 'red', show.legend = FALSE) +
  geom_label_repel(data = cities_subset[2,], aes(y=lat, x=long, label=city),
                   box.padding   = 0.35, size = 3, alpha=0.9, segment.alpha=1, point.padding = 0.5,
                   ylim=c(34.5, 36), 
                   xlim=c(-88, -86),
                   segment.color = 'red', show.legend = FALSE) +
  geom_label_repel(data = cities_subset[3,], aes(y=lat, x=long, label=city),
                   box.padding   = 0.35, size = 3, alpha=0.9, segment.alpha=1, point.padding = 0.5,
                   ylim=c(30, 31.8), 
                   xlim=c(-87, -84),
                   segment.color = 'red', show.legend = FALSE) +
  scale_size_manual(values=seq(0.07,5,by=0.07)) +
  ...
  annotation_north_arrow(location = "bl", which_north = "true", # adjust arrow location as needed
  ...
  annotate("text", x=-86, y=31.3, label="AL", color="white", hjust=0.5, vjust=0.5, size=5, angle=0) +
  annotate("text", x=-86, y=30.7, label="FL", color="white", hjust=0.5, vjust=0.5, size=5, angle=0) +
  annotate("text", x=-84.5, y=33, label="GA", color="white", hjust=0.5, vjust=0.5, size=5, angle=0) +
  annotate("text", x=-87, y=35.3, label="TN", color="white", hjust=0.5, vjust=0.5, size=5, angle=0) +
  annotate("text", x=-89, y=32, label="MS", color="white", hjust=0.5, vjust=0.5, size=5, angle=0) +
  annotate("text", x=-87.6, y=31.8, label="Alabama", color="white", hjust=0.5, vjust=0.5, size=2.5, angle=80) +
  annotate("text", x=-88.2, y=32, label="Tombigbee", color="white", hjust=0.5, vjust=0.5, size=2.5, angle=70) +
  annotate("text", x=-87.27, y=32.5, label="Cahaba", color="white", hjust=0.5, vjust=0.5, size=2.5, angle=105) +
  annotate("text", x=-86.4, y=33.5, label="Coosa", color="white", hjust=0.5, vjust=0.5, size=2.5, angle=55) +
  annotate("text", x=-87.85, y=33, label="Black Warrior", color="white", hjust=0.5, vjust=0.5, size=2.5, angle=70) +
  annotate("text", x=-85.8, y=33.1, label="Tallapoosa", color="white", hjust=0.5, vjust=0.5, size=2.5, angle=45) +
  annotate("text", x=-88.07, y=31, label="Mobile", color="white", hjust=0.5, vjust=0.5, size=2.5, angle=90) +
