...
HUCs <- c("03050207") # top-level vector of HUCs to match (must be strings, not numeric)
...
river_name <- "Combahee" # river name for files
river_name_plot <- "Combahee" # river name for plot title
...
cities_subset[1,] <- c("Georgia", "Savannah", 32.0809, -81.0912)
...
p <- ggplot() +
  ...
  annotate("text", x=-81.38, y=32.8, label="South Carolina", color="white", hjust=0.5, size=2, angle = -80) +
  annotate("text", x=-81.48, y=32.8, label="Georgia", color="white", hjust=0.5, size=2, angle = -80) +
