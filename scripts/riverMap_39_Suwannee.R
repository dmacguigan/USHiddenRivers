...
HUCs <- c("031102") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Suwannee" # river name for files
river_name_plot <- "Suwannee" # river name for plot title
...
cities_subset[1,] <- c("FL", "Gainesville", 29.6516, -82.3248)
cities_subset[2,] <- c("GA", "Valdosta", 30.8327, -83.2785)
...
p <- ggplot() +
  ...
  geom_sf(data=all_wbs_swamps_sf_crop , fill="#001204", color=NA) +
  ...
  annotate("text", x=-84, y=30.72, label="Georgia", color="white", hjust=0.5, vjust=0.5, size=3, angle=-3) +
  annotate("text", x=-84, y=30.63, label="Florida", color="white", hjust=0.5, vjust=0.5, size=3, angle=-3) +
