...
HUCs <- c("031300") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Apalachicola" # river name for files
river_name_plot <- "Apalachicola" # river name for plot title
...
target_watershed <- target_watershed[-which(as.logical(rowSums(sapply(c("03130014","03130013"), startsWith, x=huc10s)))),] # remove HUCs outside target
...
reachCodes <- as.character(target_rivers_ws$ReachCode)
target_rivers_ws <- target_rivers_ws[-which(as.logical(rowSums(sapply(c("03130014","03130013"), startsWith, x=reachCodes)))),] # remove HUCs outside target
...
cities_subset[1,] <- c("Georgia", "Atlanta", 33.7490, -84.3880)
...
gworld <- ggplot(data = world) +
  geom_sf(fill="gray20", color="white", size=0.4) +
  geom_sf(data=us_states, fill="gray20", color="white", size=0.4) +
...
p <- ggplot() +
  ...
  scale_size_manual(values=seq(0.25,5,by=0.15)) +
  ...
  annotate("text", x=-83.5, y=33, label="GA", color="white", hjust=0.5, vjust=0.5, size=6, angle=0) +
  annotate("text", x=-83.5, y=30.3, label="FL", color="white", hjust=0.5, vjust=0.5, size=6, angle=0) +
  annotate("text", x=-85.7, y=33, label="AL", color="white", hjust=0.5, vjust=0.5, size=6, angle=0) +
