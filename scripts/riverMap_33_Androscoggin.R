...
HUCs <- c("010400") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Androscoggin" # river name for files
river_name_plot <- "Androscoggin" # river name for plot title
...
cities_subset[1,] <- c("Maine", "Lewiston", 44.1004, -70.2148)
...
p <- ggplot() +
  ...
  annotate("text", x=-71.6, y=45.3, label="Quebec", color="white", hjust=0.5, vjust=0.5, size=5, angle=45) +
