...
HUCs <- c("03060201", "03060202", "03060203", "0306020402", "0306020403") # top-level vector of HUCs to match (must be strings, not numeric)
...
river_name <- "Ogeechee" # river name for files
river_name_plot <- "Ogeechee" # river name for plot title
...
gworld <- ggplot(data = world) +
  geom_sf(fill="gray20", color="white") +
  geom_sf(data=us_states, fill="gray20", color="white") +
  ...
  theme(panel.border = element_rect(colour = "gray60", fill=NA, size=2), panel.grid.major = element_blank(),
...
p <- ggplot() +
  ...
  geom_sf(data=target_watershed_sf_crop, fill="gray20", color=NA) +
  ...
  geom_sf(data=inlet_sf, fill="#82eefd", color="#82eefd") +
  ...
  #geom_point(data = capitols_subset, aes(y=lat, x=long), pch=21, size=3, stroke=2, color="white", fill="black") +
  ...
  annotation_scale(location = "br", width_hint = 0.2, unit_category="imperial", text_col="white", line_col="white") + # adjust scale location as needed
  ...
  annotate("text", x=-82.35, y=33.93, label="South Carolina", color="white", hjust=0, size=4, angle=-50) +
  annotate("text", x=-82.4, y=33.8, label="Georgia", color="white", hjust=0, size=4, angle=-50) +
...
raster::area(temp) / 1000000 
# calculate area of watershed in square miles
(raster::area(temp) / 1000000) * 0.386102
