...
HUCs <- c("030601") # top-level vector of HUCs to match (must be strings, not numeric)
...
river_name <- "Savannah" # river name for files
river_name_plot <- "Savannah" # river name for plot title
...
cities_subset <- cities_subset[2,] # get rid of Charlotte
cities_subset[2,] <- c("Georgia", "Savannah", 32.0809, -81.0912)
...
p <- ggplot() +
  ...
  geom_point(data = cities_subset, aes(y=lat, x=long), pch=21, size=3, stroke=2, color="white", fill="black") +
  ...
  annotate("text", x=-80.5, y=34.77, label="South Carolina", color="white", hjust=0, size=4) +
  annotate("text", x=-80.5, y=34.85, label="North Carolina", color="white", hjust=0, size=4) +
