...
HUCs <- c("020802") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "James" # river name for files
river_name_plot <- "James" # river name for plot title
...
location <- data.frame(loc=c("Appomattox Court House"), lat=c(37.3775), long=c(-78.7960))
...
p <- ggplot() +
  ...
  annotate("text", x=-76.1, y=37.6, label="Chesapeake Bay", color="white", hjust=0.5, vjust=0.5, size=5, angle=-90) +
