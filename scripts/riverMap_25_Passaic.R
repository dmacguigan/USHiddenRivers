...
HUCs <- c("02030103") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Passaic-Hackensack" # river name for files
river_name_plot <- "Passaic & Hackensack" # river name for plot title
...
# from https://www.sciencebase.gov/catalog/item/57753ebee4b07dd077c70868
pop <- raster(paste(wd, "/shapefiles/pden2010_block/pden2010_60m_reproj.tif", sep=""))
#pop <- rasterToPoints(pop, spatial = TRUE)
#pop <- projectRaster(pop, crs="+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
...
extents@ymax <- ymax
ymin <- floor(extents@ymin) + 0.4
extents@ymin <- ymin
xmax <- ceiling(extents@xmax) + 0.1
extents@xmax <-  xmax 
...
cities_subset[2,] <- c("New Jersey", "Newark", 40.7357, -74.1724)
cities_subset[,3] <- as.numeric(cities_subset[,3])
cities_subset[,4] <- as.numeric(cities_subset[,4])
cities_subset[1,3] <- 40.7128
cities_subset[1,4] <- -74.0060
...
pop_crop <- crop(pop, extents)
pop_crop_df <- as.data.frame(pop_crop, xy = TRUE)
pop_crop_df$log_pden2010_60m_reproj <- log10(pop_crop_df$pden2010_60m_reproj) # get log population density
pop_crop_df$log_pden2010_60m_reproj[which(pop_crop_df$log_pden2010_60m_reproj > 5)] <- 6 # replace log values greater than 6, makes plotting better
pop_crop_df$norm_log_pden2010_60m_reproj <- pop_crop_df$log_pden2010_60m_reproj/max(pop_crop_df$log_pden2010_60m_reproj) # normalize to range between 0 and 1
...
p <- ggplot() +
  ...
  geom_raster(pop_crop_df, mapping = aes(x = x, y = y, alpha = norm_log_pden2010_60m_reproj), fill="red") +
  scale_alpha(range =  c(0, 1), name="log10\npopulation\ndensity",
              labels = c(0,0.25,0.5,0.75,1.0)*max(pop_crop_df$log_pden2010_60m_reproj)) + 
  #geom_sf(data=target_watershed_sf_crop, fill=alpha("white", 0.15), color=NA) +
  geom_sf(data=rivers_sf_crop, color="#565656", lineend = "round", size=0.3, show.legend = FALSE) +
  geom_sf(data=target_wbs_lakes_all_sf_crop, fill="#565656", color=NA) +
  geom_sf(data=state_borders_sf_crop, fill=NA, color="gray", size=0.2) +
  ...
  geom_label_repel(data = cities_subset[1,], aes(y=lat, x=long, label=paste(city, ", ", state, sep="")),
                   ...
                   xlim=c(-73.7, -72), ylim=c(40.8, 42), 
                   segment.color = 'white', show.legend = FALSE) +
  geom_point(data = cities_subset[2,], aes(y=lat, x=long), pch=21, size=1, stroke=2, color="white", fill="black") +
  geom_label_repel(data = cities_subset[2,], aes(y=lat, x=long, label=paste(city, ", ", state, sep="")),
                   box.padding   = 0.35, size = 3, alpha=0.9, segment.alpha=1, point.padding = 0.5,
                   xlim=c(-75, -74.3), ylim=c(40, 40.6),
                   segment.color = 'white', show.legend = FALSE) +
  geom_label_repel(data = data.frame(y=40.826260, x =-74.038214, lab="Meadowlands"), aes(y=y, x=x, label=lab),
                   box.padding   = 0.35, size = 3, alpha=0.9, segment.alpha=1, point.padding = 0.5,
                   xlim=c(-73.8, -73), ylim=c(41, 42),
                   segment.color = 'white', show.legend = FALSE) +
  scale_size_manual(values=seq(0.3,5,by=0.15)) +
  ...
  annotate("text", x=-74.48, y=41.24, label="New Jersey", color="gray", hjust=0.5, vjust=0.5, size=3, angle=-30) +
  annotate("text", x=-74.45, y=41.27, label="New York", color="gray", hjust=0.5, vjust=0.5, size=3, angle=-30) +
  ...
  theme(panel.grid.major = element_line(color = gray(.5), linetype = "dashed", size = 0.7), 
        ...
        legend.key = element_rect("black", "black"))
