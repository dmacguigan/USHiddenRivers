...
HUCs <- c("0108") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Connecticut" # river name for files
river_name_plot <- "Connecticut" # river name for plot title
...
# get Canadian rivers in target watershed
in_ws_rivers <- over(canada_rivers_can, target_watershed)
canada_target_rivers_ws <- canada_rivers_can[!is.na(in_ws_rivers$dummy),]

# coarse (1:10 M) NA political boundaries 
# from https://www.naturalearthdata.com/downloads/10m-physical-vectors/
coarse_borders <- readOGR(paste(wd, "/shapefiles/ne_10m_land/ne_10m_land.shp", sep=""))
coarse_borders <- spTransform(coarse_borders, CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
# below code fixes some bad polygons in this shapefile
# simplify the polgons a tad (tweak 0.00001 to your liking)
coarse_borders <- gSimplify(coarse_borders, tol = 0.00001)
# this is a well known R / GEOS hack (usually combined with the above) to 
# deal with "bad" polygons
coarse_borders <- gBuffer(coarse_borders, byid=TRUE, width=0)
# any bad polys?
sum(gIsValid(coarse_borders, byid=TRUE)==FALSE)

# INLETS
# first get coarse inlet from continent shapfile
inlet_outline_coarse <- crop(coarse_borders, target_watershed)
inlet_coarse <- gDifference(target_watershed, inlet_outline_coarse)
inlet_coarse <- as(inlet_coarse, "SpatialPolygonsDataFrame" )
# then get fine scale inlet from state
# this inlet shape is problematic if the river drainage is international
inlet_outline_state <- crop(state_borders, target_watershed)
inlet_state <- gDifference(target_watershed, inlet_outline_state)
inlet_state <- as(inlet_state, "SpatialPolygonsDataFrame")
# so use the coarse inlet to subset the fine scale inlet, preserving only the real inlet area
temp <- disaggregate(inlet_state)
real_inlet <- over(temp, inlet_coarse)
inlet <- temp[!is.na(real_inlet$dummy),]
...
coastline <- gSimplify(coastline, tol = 0.00001)
...
cities_subset[2,] <- c("MA", "Springfield", 42.1015, -72.5898)
...
target_rivers_ws_sf_crop <- st_crop(target_rivers_ws_sf, xmin = xmin, xmax = xmax,
                          ymin = ymin, ymax = ymax)
canada_target_rivers_ws_sf <- st_as_sf(canada_target_rivers_ws)
canada_target_rivers_ws_sf_crop <- st_crop(canada_target_rivers_ws_sf, xmin = xmin, xmax = xmax,
                                    ymin = ymin, ymax = ymax)
...
p <- ggplot() +
  ...
  geom_sf(data=canada_target_rivers_ws_sf, size=0.2, color="#82eefd", 
          show.legend = FALSE, lineend = "round") +
  ...
  geom_point(data = cities_subset[2,], aes(y=lat, x=long), pch=21, size=1, stroke=2, color="red", fill="black") +
  geom_label_repel(data = cities_subset[2,], aes(y=lat, x=long, label=paste(city, ", ", state, sep="")),
                   box.padding   = 0.35, size = 3, alpha=0.9, segment.alpha=1, point.padding = 0.5,
                   xlim=c(-72, -72),
                   segment.color = 'red', show.legend = FALSE) + 
  ...
  annotate("text", x=-71.5, y=43.5, label="NH", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-73.3, y=41.6, label="CT", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-71.5, y=42.4, label="MA", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-72.7, y=44.5, label="VT", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-71.6, y=41.7, label="RI", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-72.2, y=45.3, label="Canada", color="white", hjust=0.5, vjust=0.5, size=5) +
