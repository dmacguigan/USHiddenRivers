...
HUCs <- c("030501") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Santee" # river name for files
river_name_plot <- "Santee" # river name for plot title
...
p <- ggplot() +
  ...
  geom_label_repel(data = cities_subset[1:2,], aes(y=lat, x=long, label=paste(city, ", ", state, sep="")),
                   box.padding   = 0.35, size = 3, alpha=0.8, segment.alpha=1, point.padding = 0.5,
                   xlim=c(-80.7, -78.5), ylim=c(35, 36.5),
                   segment.color = 'red', show.legend = FALSE) + 
  geom_label_repel(data = cities_subset[3,], aes(y=lat, x=long, label=paste(city, ", ", state, sep="")),
                   box.padding   = 0.35, size = 3, alpha=0.8, segment.alpha=1, point.padding = 0.5,
                   xlim=c(-83, -81.1), ylim=c(33, 33.8),
                   segment.color = 'red', show.legend = FALSE) + 
