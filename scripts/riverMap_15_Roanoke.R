...
HUCs <- c("030101") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Roanoke" # river name for files
river_name_plot <- "Roanoke" # river name for plot title
...
cities_subset[1,] <- c("Virginia", "Roanoke", 37.2710, -79.9414)
...
p <- ggplot() +
  ...
  geom_label_repel(data = cities_subset[1,], aes(y=lat, x=long, label=paste(city, ", ", state, sep="")),
                   ...
                   xlim=c(-79.2, -75.5), ylim=c(37.4, 38),
  ...
  annotate("text", x=-76.1, y=37.4, label="Chesapeake Bay", color="white", hjust=0.5, vjust=0.5, size=3, angle=-90) +
  annotate("text", x=-76.5, y=36.6, label="Virginia", color="white", hjust=0.5, vjust=0.5, size=3) +
  annotate("text", x=-76.5, y=36.52, label="North Carolina", color="white", hjust=0.5, vjust=0.5, size=3) +
