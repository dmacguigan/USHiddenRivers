...
HUCs <- c("03010201", "03010202", "03010203", "03010204") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Chowan" # river name for files
river_name_plot <- "Chowan" # river name for plot title
...
p <- ggplot() +
  ...
  annotate("text", x=-76.1, y=37.6, label="Chesapeake Bay", color="white", hjust=0.5, vjust=0.5, size=3, angle=-90) +
  annotate("text", x=-78.5, y=36.58, label="Virginia", color="white", hjust=0.5, vjust=0.5, size=3) +
  annotate("text", x=-78.5, y=36.51, label="North Carolina", color="white", hjust=0.5, vjust=0.5, size=3) +
