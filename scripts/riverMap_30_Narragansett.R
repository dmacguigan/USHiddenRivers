...
HUCs <- c("01090004", "01090003") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Narragansett" # river name for files
river_name_plot <- "Narragansett" # river name for plot title
...
cities_subset[1,] <- c("RI", "Providence", 41.8240, -71.4128)
cities_subset[,3] <- as.numeric(cities_subset[,3])
cities_subset[,4] <- as.numeric(cities_subset[,4])

#  create special polygon to crop the inlet and watershed
xy <- data.frame(x=c(xmin, xmax, xmax, xmin, xmin), 
                 y=c(ymax, ymax, 41.82, 41.2, ymax))
xy$ID <- rep(1, nrow(xy))
...
st_crs(poly_crop) <- st_crs(inlet_sf)
inlet_sf_crop <- st_intersection(inlet_sf, poly_crop, st_crs(inlet_sf))[1]
...
st_crs(poly_crop) <- st_crs(target_watershed_sf)
target_watershed_sf_crop <- st_intersection(target_watershed_sf, poly_crop, st_crs(target_watershed_sf))[1]
...
p <- ggplot() +
  ...
  annotate("text", x=-72, y=41.7, label="CT", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-71.3, y=42.3, label="MA", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-71.6, y=41.5, label="RI", color="white", hjust=0.5, vjust=0.5, size=5) +
  labs(title = paste(river_name_plot, "Bay Basin")) +
  coord_equal() +
