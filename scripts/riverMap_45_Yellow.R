...
HUCs <- c("03140103") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Yellow" # river name for files
river_name_plot <- "Yellow" # river name for plot title
...
# get R. okaloosae locality info
setwd(wd)
Roka <- read.csv("./data/Rana_okaloosae_vertnet.csv", header=TRUE)
...
p <- ggplot() +
  ...
  geom_point(data = Roka, aes(y=lat, x=long, color=sp, fill=sp), pch=21, size=1)+
  scale_color_manual(values = c("white"), name=NULL, labels=c(bquote(italic("Rana okaloosae")))) +
  scale_fill_manual(values = c("red"), name=NULL, labels=c(bquote(italic("Rana okaloosae")))) +
