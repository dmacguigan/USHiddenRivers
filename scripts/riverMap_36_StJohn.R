...
HUCs <- c("82616", "221316") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "SaintJohn" # river name for files
river_name_plot <- "Saint John" # river name for plot title
...
watersheds <- readOGR(paste(wd, "/shapefiles/Watersheds_Shapefile_NA_Watersheds_data_NA_Watersheds_watershed_p_v2/Watersheds_Shapefile/NA_Watersheds/watershed_p_v2.shp", sep=""))
watersheds <- spTransform(watersheds, CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
UIDENT <- as.character(watersheds$UIDENT)
target_watershed <- watersheds[which(as.logical(rowSums(sapply(HUCs, startsWith, x=UIDENT)))),]
...
rivers <- readOGR(paste(wd, "/shapefiles/river_network_variable_Dd/pfaf_variable_Dd_72.shp", sep=""))
...
target_rivers_ws <- rivers[!is.na(in_ws_rivers),]
...
# gets rivers outside watershed boundary
in_ws_rivers <- over(rivers_US, target_watershed)
rivers_US_not_ws <- rivers_US[is.na(in_ws_rivers),]
...
target_wbs_lakes <- target_wbs_lakes_all[!is.na(in_ws_lakes),]
names <- as.character(target_wbs_lakes$Name)
target_wbs_lakes <- target_wbs_lakes_all[which(!as.logical(rowSums(sapply(c("Chamberlain Lake"), grepl, x=names, fixed=TRUE)))),] #remove one problematic lake
...
target_wbs_swamps <- target_wbs_swamps[!is.na(in_ws_swamps),]
...
in_ws_can_lakes <- over(canada_lakes_can, target_watershed)
target_wbs_can_lakes <- canada_lakes_can[!is.na(in_ws_can_lakes),]
target_wbs_can_lakes <- aggregate(target_wbs_can_lakes, dissolve=T)
# below code fixes some bad polygons in this shapefile
# simplify the polgons a tad (tweak 0.00001 to your liking)
target_wbs_can_lakes <- gSimplify(target_wbs_can_lakes, tol = 0.00001)
target_wbs_can_lakes <- as(target_wbs_can_lakes, "SpatialPolygonsDataFrame" )
setwd(paste(wd, "/shapefiles/isolatedWaterbodies", sep=""))
writeOGR(target_wbs_can_lakes, dsn = '.', layer = paste(river_name, "_waterbodies_can_lakes", sep=""), driver = "ESRI Shapefile", overwrite_layer=TRUE)
...
# gets rivers outside watershed boundary
in_ws_rivers <- over(canada_rivers_can, target_watershed)
canada_rivers_can_not_ws <- canada_rivers_can[is.na(in_ws_rivers),]
...
cities_subset[1,] <- c("NB", "St. John", 45.275095, -66.058517)
...
target_wbs_lakes_sf_crop <- st_crop(target_wbs_lakes_sf, xmin = xmin, xmax = xmax,
                                 ymin = ymin, ymax = ymax)
target_wbs_can_lakes_sf <- st_as_sf(target_wbs_can_lakes)
target_wbs_can_lakes_sf_crop <- st_crop(target_wbs_can_lakes_sf, xmin = xmin, xmax = xmax,
                                    ymin = ymin, ymax = ymax)
...
target_watershed_sf_crop <- st_crop(target_watershed_sf, xmin = xmin,  xmax = xmax,
                                    ymin = ymin, ymax = ymax) 
rivers_sf <- st_as_sf(rivers_US_not_ws)
...
canada_lakes_can_sf_crop <- st_crop(canada_lakes_can_sf, xmin = xmin, xmax = xmax,
                                    ymin = ymin, ymax = ymax)
canada_rivers_can_sf <- st_as_sf(canada_rivers_can_not_ws)
...
target_rivers_ws_sf_crop$strmOrder[target_rivers_ws_sf_crop$strmOrder<0] <- 1

# remove stream order values <4 for target river
# just makes the plot less dense
target_rivers_ws_sf_crop <- target_rivers_ws_sf_crop[target_rivers_ws_sf_crop$strmOrder>2,]
target_rivers_ws_sf_crop$strmOrder <- as.numeric(target_rivers_ws_sf_crop$strmOrder) - 2
...
gworld <- ggplot(data = world) +
  ...
  coord_sf(xlim = c(-130, -60), ylim = c(25, 55), expand = FALSE, datum=NA) +
...
p <- ggplot() +
  ...
  geom_sf(data=target_wbs_can_lakes_sf_crop, fill="#82eefd", color=NA) +
  geom_sf(data=target_rivers_ws_sf_crop, aes(size=factor(strmOrder)), color="#82eefd",
  ...
  annotate("text", x=-70.55, y=45.8, label="Quebec", color="white", hjust=0.5, vjust=0.5, size=5, angle=50) +
  annotate("text", x=-66, y=47, label="New\nBrunswick", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-69, y=45.5, label="Maine", color="white", hjust=0.5, vjust=0.5, size=5) +
  annotate("text", x=-65.7, y=45, label="Bay of Fundy", color="white", hjust=0.5, vjust=0.5, size=5, angle=30) +
