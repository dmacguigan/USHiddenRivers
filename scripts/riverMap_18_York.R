...
HUCs <- c("02080107", "02080105", "02080106") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "York" # river name for files
river_name_plot <- "York" # river name for plot title
...
p <- ggplot() +
  ...
  geom_label_repel(data = cities_subset[1,], aes(y=lat, x=long, label=paste(city, ", ", state, sep="")),
                   ...
                   xlim=c(-79, -77.4), ylim=c(36, 37.8),
                   segment.color = 'red', show.legend = FALSE) + 
  geom_point(data = locations, aes(y=lat, x=long), pch=21, size=1, stroke=2, color="red", fill="black") +
  geom_label_repel(data = locations, aes(y=lat, x=long, label=loc),
                   box.padding   = 0.35, size = 3, alpha=0.8, segment.alpha=1, point.padding = 0.5,
                   xlim=c(-79, -76.7), ylim=c(36, 37.2),
                   segment.color = 'red', show.legend = FALSE) + 
