...
HUCs <- c("03100208") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Withlacoochee " # river name for files
river_name_plot <- "Withlacoochee" # river name for plot title
...
cities_subset[1,] <- c("Florida", "Rainbow Springs", 29.101551, -82.436641)
...
p <- ggplot() +
  ...
  geom_sf(data=all_wbs_swamps_sf_crop , fill="#002107", color=NA) +
