...
HUCs <- c("020700") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Potomac" # river name for files
river_name_plot <- "Potomac" # river name for plot title
...
p <- ggplot() +
  ...
  annotate("text", x=-79.8, y=38.2, label="Virginia", color="white", hjust=0.5, vjust=0.5, size=3, angle=60) +
  annotate("text", x=-79.87, y=38.31, label="West Virginia", color="white", hjust=0.5, vjust=0.5, size=3, angle=60) +
