...
HUCs <- c("03020101","03020102","03020103",
          "0302010401", "0302010402") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Tar" # river name for files
river_name_plot <- "Tar" # river name for plot title
...
sp_data <- read.csv(paste(wd, "/data/Lythrurus_matutinus_fishnet2.csv", sep=""), header=TRUE)
...
cities_subset[1,] <- c("North Carolina", "Rocky Mount", 35.9382, -77.7905)
...
p <- ggplot() +
  ...
  geom_point(data = cities_subset, aes(y=lat, x=long), pch=21, size=1, stroke=2, color="white", fill="black") +
  geom_label_repel(data = cities_subset[1,], aes(y=lat, x=long, label=paste(city, ", ", state, sep="")),
                   ...
                   xlim=c(-77.3, -75.5), ylim=c(36.1, 37),
                   segment.color = 'red', show.legend = FALSE) + 
  geom_point(data = sp_data, aes(y=Latitude, x=Longitude, color=ScientificName, fill=ScientificName), pch=21, size=0.8, alpha=0.7)+
  scale_color_manual(values = c("white"), name=NULL, labels=c(bquote(italic("Lythrurus matutinus")))) +
  scale_fill_manual(values = c("red"), name=NULL, labels=c(bquote(italic("Lythrurus matutinus")))) +
  ...
  annotation_scale(location = "br", width_hint = 0.2, unit_category="metric",  text_col="white", line_col="white") + # adjust scale location as needed
  ...
  annotate("text", x=-76.1, y=36.04, label="Albemarle Sound", color="white", hjust=0.5, vjust=0.5, size=3) +
