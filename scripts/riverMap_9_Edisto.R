...
HUCs <- c("03050206", "03050204", "03050205", "03050203") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
states <- c("SC") # states spanned by the river basin (for parsing waterbodies), use abbreviations
river_name <- "Edisto" # river name for files
river_name_plot <- "Edisto" # river name for plot title
...
# lakes only
target_wbs_lakes_all <- wbs[which(as.logical(rowSums(sapply(c("Lake"), grepl, x=feature, fixed=TRUE)))),]
...
cities_subset[1,] <- c("South Carolina", "Charleston", 32.7765, -79.9311)
...
p <- ggplot() +
  ...
  scale_size_manual(values=seq(0.2,2,by=0.1)) +
  ...
  annotate("text", x=-81.38, y=32.8, label="South Carolina", color="white", hjust=0.5, size=3, angle = -80) +
  annotate("text", x=-81.48, y=32.8, label="Georgia", color="white", hjust=0.5, size=3, angle = -80) +
