...
HUCs <- c("031001") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Peace" # river name for files
river_name_plot <- "Peace and Myakka" # river name for plot title
...
in_ws_swamps <- over(all_wbs_swamps, target_watershed)
all_wbs_swamps <- all_wbs_swamps[is.na(in_ws_swamps),]
# below code fixes some bad polygons in this shapefile
# simplify the polgons a tad (tweak 0.00001 to your liking)
...
cities_subset[1,] <- c("Florida", "Port Charlotte", 26.9762, -82.0906)
cities_subset[2,] <- c("Florida", "Fort Myers", 26.6406, -81.8723)
...
p <- ggplot() +
  ...
  geom_sf(data=all_wbs_swamps_sf_crop , fill="#292929", color=NA) +
  ...
  annotate("text", x=-82.31, y=27.03, label="Myakka", color="white", hjust=0.5, vjust=0.5, size=2, angle=-45) +
  annotate("text", x=-82.06, y=26.99, label="Peace", color="white", hjust=0.5, vjust=0.5, size=2, angle=10) +
