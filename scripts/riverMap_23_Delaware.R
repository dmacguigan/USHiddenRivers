...
HUCs <- c("020402", "020401") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Delaware" # river name for files
river_name_plot <- "Delaware" # river name for plot title
...
# any bad polys?
sum(gIsValid(wbs, byid=TRUE)==FALSE)
...
cities_subset <- cities_subset[c(4,7),]
...
p <- ggplot() +
  ...
  annotate("text", x=-76.5, y=41.96, label="Pennsylvania", color="white", hjust=0.5, vjust=0.5, size=3) +
  annotate("text", x=-76.5, y=42.05, label="New York", color="white", hjust=0.5, vjust=0.5, size=3) +
  annotate("text", x=-74.4, y=41.18, label="New Jersey", color="white", hjust=0.5, vjust=0.5, size=3, angle=-30) +
  annotate("text", x=-74.35, y=41.25, label="New York", color="white", hjust=0.5, vjust=0.5, size=3, angle=-30) +
  annotate("text", x=-75.65, y=38.8, label="Delaware", color="white", hjust=0.5, vjust=0.5, size=3, angle=-85) +
  annotate("text", x=-75.76, y=38.8, label="Maryland", color="white", hjust=0.5, vjust=0.5, size=3, angle=-85) +
