...
HUCs <- c("031800") # top-level vector of HUCs to match (must be strings, not numeric)
# search for HUCs here: https://water.usgs.gov/wsc/map_index.html
river_name <- "Pearl" # river name for files
river_name_plot <- "Pearl" # river name for plot title
...
cities_subset[1,] <- c("MS", "Jackson", 32.2988, -90.1848)
...
gworld <- ggplot(data = world) +
  geom_sf(fill="gray20", color="white", size=0.15) +
  geom_sf(data=us_states, fill="gray20", color="white", size=0.15) +
...
p <- ggplot() +
  ...
  #geom_raster(topo_crop_df, mapping = aes(x = x, y = y, fill=GRAY_HR_SR_OB, alpha = GRAY_HR_SR_OB)) +
  #scale_fill_gradient(low="black", high="gray30", guide = "none") +
  ...
  geom_sf(data=landBorders_sf_crop, fill=NA, color="white", size=0.5) +
  ...
  scale_size_manual(values=seq(0.12,5,by=0.1)) +
  ...
  annotate("text", x=-90.7, y=30.96, label="Louisiana", color="white", hjust=0.5, vjust=0.5, size=3, angle=0) +
  annotate("text", x=-90.7, y=31.03, label="Mississippi", color="white", hjust=0.5, vjust=0.5, size=3, angle=0) +
